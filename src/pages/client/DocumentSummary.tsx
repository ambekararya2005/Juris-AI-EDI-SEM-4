...
const DocumentSummary: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    setAnalyzing(false);
    setAnalyzed(true);
    // Keep skeleton for 2 seconds then reveal
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className={`grid gap-6 ${analyzed ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 max-w-lg mx-auto'}`}>
        ...
        <Card className="p-6 dark:bg-slate-800 dark:border-slate-700">
          ...
          >
            ...
            {uploadState === 'ready' && (
              <div className="space-y-2">
                ...
                <p className="text-sm font-medium text-dark-text dark:text-slate-100">Supplier Agreement – TechNova Pvt. Ltd. vs. Zenith Traders.pdf</p>
                <p className="text-xs text-muted-text dark:text-slate-400">2.4 MB • PDF</p>
              </div>
            )}
          </div>
          ...
          <p className="text-xs text-muted-text dark:text-slate-400 mt-3 text-center">Supported: PDF, DOCX, DOC, TXT</p>
        </Card>
