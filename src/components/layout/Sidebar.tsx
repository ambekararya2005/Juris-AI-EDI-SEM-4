...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside
      className={`relative flex flex-col bg-navy text-white transition-all duration-300 ease-in-out
    ...
    >
      ...
      {/* User + Logout */}
      <div className={`border-t border-white/10 p-4 ${collapsed ? 'flex flex-col items-center gap-3' : ''}`}>
        {!collapsed && user && (
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-full bg-gold flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
              {user.name.charAt(0)}
            </div>
      ...
      <button
        ...
        className="absolute -right-3 top-20 w-6 h-6 bg-navy border border-white/20
          rounded-full flex items-center justify-center text-white hover:bg-blue-brand transition-colors z-10"
